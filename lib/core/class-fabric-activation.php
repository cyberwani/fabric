...
class Fabric_Activation
{
	...
	{
		require_once FABRIC_ACTIVATION_DIR . 'helper.php';
		
		if ( is_admin() && get_option( 'fabric-plugin-notifications' ) ) {
			require_once FABRIC_ACTIVATION_DIR . 'tgmpa-plugin-notifications.php';
	...
	public function customizer( $wp_customize )
	{
		...
		$wp_customize->add_setting( 'fabric-packages', array(
			'default'		=> '',
			'type'			=> 'option',
		...
		$wp_customize->add_setting( 'fabric-plugin-notifications', array(
			'default'		=> true,
			'type'			=> 'option',
			'transport'		=> 'postMessage',
			'capability'	=> 'edit_theme_options'
		...
		$wp_customize->get_section('static_front_page')->title = __( 'Permalinks & Static Home Page', 'fabric' );
		$wp_customize->get_section('static_front_page')->description = '';
		$wp_customize->get_section('static_front_page')->priority = 30;
		$wp_customize->add_setting( 'fabric-permalinks', array(
			'default'		=> false,
			'type'			=> 'option',
			'transport'		=> 'postMessage',
			'capability'	=> 'edit_theme_options'
		...
		$wp_customize->add_setting( 'fabric-nice-search', array(
			'default'		=> false,
			'type'			=> 'option',
			'transport'		=> 'postMessage',
			'capability'	=> 'edit_theme_options'
		...
		$wp_customize->add_setting( 'fabric-nav-menu', array(
			'default'		=> false,
			'type'			=> 'option',
			'transport'		=> 'postMessage',
			'capability'	=> 'edit_theme_options'
